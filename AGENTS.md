...
- **Static site (main product):** hand-written HTML + vanilla JS at the repo root (`index.html`, `schedule.html`, `report.html`, `dday.html`) plus dashboards under `kstartup/`, `tmap/`, and `seoul-city/`. No bundler. Serve the repo root statically.
...
- **`api/seoul-citydata.js`:** Seoul Open Data citydata proxy (MCP-style actions). Optional `SEOUL_OPEN_API_KEY`; without it the sample key only returns 광화문·덕수궁. The dashboard also calls the Seoul API directly from `http://localhost:3000`.
...
- **Test:** `npm run test:kstartup` and `npm run test:seoul-city` (`node:assert` parser tests; no server needed).
...
- Optional env vars (`DATA_GO_KR_API_KEY`, `SEOUL_OPEN_API_KEY`, `SUPABASE_ACCESS_TOKEN`/`DATABASE_URL`, `VERCEL_TOKEN`) are only needed for fetching fresh K-Startup/Seoul city data, applying DB schemas, or deploying — not for local viewing/testing.
