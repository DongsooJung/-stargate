#!/usr/bin/env node
/**
 * Supabase에 K-Startup 스키마를 적용합니다.
 */
...
const SQL = readFileSync(join(__dirname, '../supabase/kstartup.sql'), 'utf8');
...
function applySqlViaPsql() {
  ...
  const result = spawnSync('psql', [DB_URL, '-v', 'ON_ERROR_STOP=1', '-f', 'supabase/kstartup.sql'], {
    encoding: 'utf8',
  });
  if (result.status === 0) {
...
async function probeTable() {
  ...
  if (!key) return false;
  const response = await fetch(`${SUPABASE_URL}/rest/v1/kstartup_announcements?select=id&limit=1`, {
    headers: {
      apikey: key,
